...
class Scanner:
    def __init__(self, problem):
        # TODO: the scanner should make use of  the grammar.txt file
        with open("tokens.in") as file:
            self.__tokens = [line.strip() for line in file.readlines()]
        self.__symbol_table = SymbolTable(200)
        self.__pif = []
    ...
    def __scan(self):
        ...
        for index, line in enumerate(self.__problem_text):
            ...
            for w in words:
                ...
                # this is the original line, not sure if comments should be in PIF as they are for devs only
                # if w.startswith('//') or w in self.__tokens:

                if w.startswith('//'):
                    continue
                elif w in self.__tokens:
    ...
    def breakup_line(line):
        ...
        while pos < len(line):
            ...
            if c == "\"":
                ...
                if found is False:
                    words.append(line[pos:len(line) - 1])
                    return words
