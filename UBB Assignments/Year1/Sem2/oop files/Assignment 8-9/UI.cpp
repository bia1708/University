...
void UI::runApp()
{
    ...
    std::cin >> type;
    this->userService.repositoryType(type);
    while (1) {
...
void UI::uiAdminList()
{
    std::vector<Event> listOfEvents = adminService.adminGetEventList();
...
{
    for (int i = 0; i < userService.getInterests().size(); i++)
        std::cout << userService.getInterests()[i].getOutputFormat() << '\n';
}

void UI::uiUserOpenApp(){
    this->userService.openInterestListInApp();
}
...
void UI::uiUserFilter() {
    ...
    int month;
    Event currentEvent;
    std::cout << "Please enter the month(or enter 0 to get all events): ";
    std::cin >> consoleInput;
