...
DynamicVector<Event> AdminService::adminGetEventList()
{
...
AdminService initTestService() {
	...
	serv.adminAddEvent("Party1", "Descr1", date, 100, "link");

	date = { 14, 5, 2022, 11, 30 };
	serv.adminAddEvent("Party2", "Descr2", date, 200, "link");
...
void testAdminService() {
	AdminService serv = initTestService();

	Date date{ 12, 3, 2021, 14, 30 };
	assert(serv.adminAddEvent("Party3", "Descr3", date, 300, "link"));
	assert(serv.adminGetEventList().getSize() == 3);

	assert(serv.adminDeleteEvent("Party1"));
	assert(serv.adminGetEventList().getSize() == 2);

	assert(serv.adminUpdateEvent("Party3", "UPDATE", date, 400, "link"));
}
