...
from ford import ford_min_cost_walk, FordException
import os
...
class Console:
    ...
    # 25
    def ford_lowest_cost_walk(self):
        try:
            start = int(input("Starting point: "))
            end = int(input("End point: "))
            dist, lowest_cost_path = ford_min_cost_walk(self.__graph, start, end)
            if len(lowest_cost_path) == 0:
                print("There is no path between the vertices.\n")
            elif dist is None:
                print("Negative cost cycle!\n")
            else:
                print(f"Distance: {dist}\n")
                str = ""
                for vertex in lowest_cost_path:
                    str += f"{vertex}" + " -> "
                str = str[:len(str) - 4]
                print(str)
        except ValueError:
            print("Invalid input!")
        except FordException as fe:
            print(fe)
