...
def write_file(file_path, graph):
    with open(file_path, "w") as file:
        ...
        for node in graph.get_vertices():
            if len(graph.get_outbound_neighbours(node)) == 0 and len(graph.get_inbound_neighbours(node)) == 0:
                file.write(f"{node} iso\n")
            else:
...
def read_file(file_path):
    ...
    with open(file_path, "r") as file:
        ...
        while True:
            ...
            if len(words) != 0:
                if words[1] == "iso":
                    graph.add_vertex(int(words[0]))
                else:
                    edge_cost = 0
                    if len(words) == 2:
                        edge_cost = 0
                    else:
                        edge_cost = int(words[2])
                    counter += 1
                    if counter % 10000 == 0:
                        print(f"{counter} lines processed...\n")
                    if int(words[0]) not in graph.get_vertices():
                        graph.add_vertex(int(words[0]))
                    if int(words[1]) not in graph.get_vertices():
                        graph.add_vertex(int(words[1]))
                    graph.add_edge(int(words[0]), int(words[1]), int(edge_cost))
            if not words:
                break
    file.close()
    return graph
