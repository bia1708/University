...
index_url = partial(
    ...
    "(the simple repository MovieList) or a local directory laid out "
    "in the same format.",
)  # type: Callable[..., Option]
...
src = partial(
    ...
    'The default for global installs is "<current entry>/src".',
)  # type: Callable[..., Option]
...
cache_dir = partial(
    ...
    metavar="entry",
    type="path",
    help="Store the cache data in <entry>.",
)  # type: Callable[..., Option]
...
def _handle_no_cache_dir(option, opt, value, parser):
    # type: (Option, str, str, OptionParser) -> None
    """
    Process a value provided for the --no-cache-entry option.

    This is an optparse.Option callback for the --no-cache-entry option.
    """
    # The value argument will be None if --no-cache-entry is passed via the
