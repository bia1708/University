...
class Rental:
    ...
    def line_to_rental(line):
        words = line.split(", ")
        if len(words) is 4:
            return Rental(int(words[0]), int(words[1]), int(words[2]), datetime.datetime.strptime(words[3], "%d-%m-%Y"),
                          datetime.datetime.strptime(words[4], "%d-%m-%Y"), datetime.datetime.strptime(words[5], "%d-%m-%Y"))
        else:
            return Rental(int(words[0]), int(words[1]), int(words[2]), datetime.datetime.strptime(words[3], "%d-%m-%Y"),
                          datetime.datetime.strptime(words[4], "%d-%m-%Y"))

    @staticmethod
    def rental_to_line(rental):
        if rental.return_date is not None:
            return f"{rental.id}, {rental.movie_id}, {rental.client_id}, {rental.rental_date.strftime('%d-%m-%Y')}, " \
                   f"{rental.due_date.strftime('%d-%m-%Y')}, {rental.return_date.strftime('%d-%m-%Y')}"
        else:
            return f"{rental.id}, {rental.movie_id}, {rental.client_id}, {rental.rental_date.strftime('%d-%m-%Y')}, " \
                   f"{rental.due_date.strftime('%d-%m-%Y')}"
